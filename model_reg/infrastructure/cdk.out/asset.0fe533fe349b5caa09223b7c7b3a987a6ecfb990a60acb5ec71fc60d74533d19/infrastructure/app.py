...
try:
    ...
    logging.basicConfig(
        format=json.dumps({"timestamp": "%(asctime)s", "message": "%(message)s"})
    )
...
def upload_model(event, correlation_id):
    ...
    if not model_id or not model_data:
        ...
        return {
            "statusCode": 400,
            "body": json.dumps({"error": "Missing model_id or data"}),
        }
    ...
    try:
        ...
        logger.info(
            f"CorrelationID {correlation_id}: Successfully uploaded model {model_id}"
        )
        return {
            "statusCode": 201,
            "body": json.dumps({"message": "Model uploaded successfully"}),
        }
...
def list_models(correlation_id):
    ...
    try:
        response = table.scan()
        logger.info(
            f"CorrelationID {correlation_id}: Retrieved {len(response.get('Items', []))} models."
        )
        ...
        return {
            "statusCode": 500,
            "body": json.dumps({"error": "Failed to list models"}),
        }
...
def delete_model(event, correlation_id):
    ...
    try:
        ...
        s3_client.delete_object(
            Bucket=os.getenv("S3_BUCKET_NAME"), Key=f"models/{model_id}"
        )
        ...
        logger.info(
            f"CorrelationID {correlation_id}: Successfully deleted model {model_id}."
        )
        ...
        return {
            "statusCode": 500,
            "body": json.dumps({"error": "Failed to delete model"}),
        }
...
def health_check():
    ...
    return {
        "statusCode": 200,
        "body": json.dumps(
            {"status": "healthy", "time": datetime.utcnow().isoformat()}
        ),
    }
