...
class PackageService:
    ...
    def update_package_metadata(self, package_id: str, updates: Dict) -> Optional[Package]:
        ...
        allowed_fields = ['description', 'author', 'license', 'model_card', 'tags', 
                          'rating_score', 'reproducibility_score', 'reviewedness_score', 'tree_score']
    ...
    def list_packages(self, name_regex: Optional[str] = None, 
        ...
        try:
            ...
            for p in packages:
                if (search_lower in p.metadata.name.lower() or
                    (p.metadata.model_card and search_lower in p.metadata.model_card.lower())):
                    filtered.append(p)
